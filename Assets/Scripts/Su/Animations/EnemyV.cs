...
public class EnemyV : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            else
            {
                    ...
                    */
                    ...
                    if (!Target_obj)
                    {
                        ...
                        if (nearest_obj)
                        {
                            ...
                            Patrol_index = ScriptGO.GetComponent<GameSceneScript>().Patrol_list.IndexOf(Target_obj);
                        }
                ...
                {
                    Target_obj = ScriptGO.GetComponent<GameSceneScript>().Patrol_list[Patrol_index];
                    _nav.speed = 5.8f;
                    bool Have_player = false;
                    GameObject[] players = GameObject.FindGameObjectsWithTag("Player");
                    //Debug.Log(players + " : players, players.length : " + players.Length);
                    //check distance for every single player object picked 
                    if (players.Length >= 1)
                    {
                        foreach (GameObject player in players)
                    ...
                    else
                    {
                        ...
                        //Debug.Log(Vector3.Distance(Target_obj.transform.position, transform.position) + "quek");
                        if (Vector3.Distance(Target_obj.transform.position, transform.position) > 0.5f)
                        {
                            Debug.Log("Patrol VampireV : " + gameObject.name);
    ...
    {
        if (ScriptGO.GetComponent<GameSceneScript>().Enemy_Left == 1)
        {
            GoldenGoose.SetActive(true);
        }
        ScriptGO.GetComponent<GameSceneScript>().Enemy_Left -= 1;
        HealthBar_canvas.SetActive(false);
        AnigameObject.SetActive(false);
        gameObject.tag = "Untagged";
